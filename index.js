...
function populateRoundAnswers(
  ...
  ANSWER_COUNT = 4;
  if (translatedQuestion['type'] === "Fill-In") ANSWER_COUNT = 1;
  if (translatedQuestion['type'] === "Boolean") ANSWER_COUNT = 2;
  Object.assign(ANSWER_COUNT, ANSWER_COUNT);
  console.log(ANSWER_COUNT);
  let index = answersCopy.length;

  if (index < ANSWER_COUNT) {
    console.log(`The answerCopy for the answer count is ${index}.`);
...
function isAnswerLiteralCorrect(intent, text){
  console.log(text);
  console.log("isAnswerLiteralCorrect");
  console.log(intent);
  try {
...
function handleUserGuess(userGaveUp, handlerInput) {
  ...
  if (answerSlotValid
    && !intent.slots.AnswerLiteral) {
    if (parseInt(intent.slots.Answer.value, 10) === sessionAttributes.correctAnswerIndex){
      currentScore += 1;
  ...
  } else if ( isAnswerLiteralCorrect(intent, correctAnswerText) ) {
    currentScore += 1;
    speechOutputAnalysis = requestAttributes.t('ANSWER_CORRECT_MESSAGE');
  } else {
    if (!userGaveUp) {
  ...
  ANSWER_COUNT = 4;
  if (translatedQuestions[gameQuestions[currentQuestionIndex]]['type'] === "Fill-In") ANSWER_COUNT = 1;
  if (translatedQuestions[gameQuestions[currentQuestionIndex]]['type'] === "Boolean") ANSWER_COUNT = 2;
  Object.assign(ANSWER_COUNT, ANSWER_COUNT);
  ...
  if (translatedQuestions[gameQuestions[currentQuestionIndex]]['type'] !== 'Fill-In'){
    for (let i = 0; i < ANSWER_COUNT; i += 1) {
...
function startGame(newGame, handlerInput) {
  ...
  if (translatedQuestions[gameQuestions[0]]['type'] === "Boolean") ANSWER_COUNT = 2;
  if (translatedQuestions[gameQuestions[0]]['type'] === "Fill-In") ANSWER_COUNT = 1;
  Object.assign(ANSWER_COUNT, ANSWER_COUNT);
...
const languageString = {
  en: {
    translation: {
      ...
      TRIVIA_UNHANDLED: 'Try saying a number between 1 and %s or if it\'s a fill in the blank, say the blank value',
      HELP_UNHANDLED: 'Say yes to continue, or no to end the game.',
      START_UNHANDLED: 'Say start to start a new game.',
      NEW_GAME_MESSAGE: 'Welcome to %s. ',
      WELCOME_MESSAGE: 'I will ask you %s questions, try to get as many right as you can. Just say the number of the answer or an answer. Let\'s begin. ',
      ANSWER_CORRECT_MESSAGE: 'correct. ',
      ANSWER_WRONG_MESSAGE: 'wrong. ',
      CORRECT_ANSWER_MESSAGE: 'The correct answer is %s: %s. ',
      ANSWER_IS_MESSAGE: 'That answer is ',
      TELL_QUESTION_MESSAGE: 'Question %s. %s ',
      GAME_OVER_MESSAGE: 'You got %s out of %s questions correct. Thank you for playing Fire trivia!',
  ...
  'en-US': {
    translation: {
      QUESTIONS: questions.QUESTIONS_EN_US,
      GAME_NAME: 'American FHIR Trivia'
    },
  }
};
